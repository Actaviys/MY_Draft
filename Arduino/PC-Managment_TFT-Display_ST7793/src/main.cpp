...
Adafruit_GFX_Button volume_slider_btn;// Кнопки для слайдера
...
// Перевірки натискання на кнопки
  ...
  if (on_off_btn.justPressed()) { // ON/OFF
    ...
    if (ButtFlag_OnOff == 1) tft.fillRect(Rect1_PosX, Rect1_PosY, Rect1_Width, Rect1_Height, RED); else tft.fillRect(Rect1_PosX, Rect1_PosY, Rect1_Width, Rect1_Height, GREEN);
